...
import VanillaExpansion.expand.special.*;
import mindustry.*;
...
public class VESFX implements ApplicationListener{
    public static VESFX inst;
    ...
    void draw(){
        ...
        if(!blackHoleQueue.isEmpty()){
            VEShaders.blackholeShader.holes.addAll(blackHoleQueue);
            Draw.draw(Layer.floor - 1f, () -> buffer.begin(Color.clear));
            Draw.draw(Layer.blockOver + 0.1f, () -> {
                buffer.end();
                buffer.blit(VEShaders.blackholeShader);
                VEShaders.blackholeShader.holes.clear();
            });
        ...
        if(impFrameTime > 0f && !impFrameEntities.isEmpty()){
            Draw.draw(Layer.end - 1, () -> {
                ...
                VEShaders.harshShadow.clear();
                for(ImpactFrameDrawer impact : impFrameEntities){
                    float x = impact.x;
                    float y = impact.y;
                    VEShaders.harshShadow.addLight(x, y, impact.intensity);
                ...
                Draw.shader(VEShaders.alphaCut);
                ...
                buffer.end();
                buffer.blit(VEShaders.harshShadow);
    ...
    public void drawOrderPortal(float x, float y, float width, float height){
        Draw.draw(Layer.flyingUnitLow - 0.5f, () -> {
            VEShaders.orderShader.width = width;
            VEShaders.orderShader.height = height;
            VEShaders.orderShader.srcX = x;
            VEShaders.orderShader.srcY = y;

            Draw.shader(VEShaders.orderShader);
            Draw.shader();
        });
    }
    public void drawChaosPortal(float x, float y, float width, float height){
        Draw.draw(Layer.flyingUnitLow - 0.5f, () -> {
            VEShaders.chaosShader.width = width;
            VEShaders.chaosShader.height = height;
            VEShaders.chaosShader.srcX = x;
            VEShaders.chaosShader.srcY = y;

            //Draw.flush();
            Draw.shader(VEShaders.chaosShader);
            //Draw.flush();
            Draw.shader();
        });
    }
