...
/** Ported from Project Unity's ExpBulletType: bullets that credit their owner turret with exp on hit. */
public class ExpBulletType extends BulletType{
    ...
    public void drawLight(Bullet b){
        ...
        Drawf.light(b.x, b.y, lightRadius, overrideLight ? getColor(b).mul(lightColor) : lightColor, lightOpacity);
    }
    ...
    public void handleExp(Bullet b, float x, float y, int amount){
        ...
        if(b.owner instanceof ExpTurret.ExpTurretBuild exp){
            if(exp.level() < exp.maxLevel()){
                for(int i = 0; i < Math.ceil(amount); i++){
                    ExpFx.expGain.at(x, y, 0f, b.owner);
                }
            }
