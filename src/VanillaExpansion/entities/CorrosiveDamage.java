...
public class CorrosiveDamage {
    ...
    public static void tileDamage(Team team, int tx, int ty, float baseRadius, float damage, @Nullable Bullet source){
        Time.run(0f, () -> {
            ...
            if(in != null && in.team != team && in.block.size > 1 && in.health > damage && !(in instanceof TantalumWall.TantalumWallBuild)){
                ...
                if(source != null){
                    if(source.type.status != StatusEffects.corroded) in.damage(source, team, d);
                }else{
                    in.damage(team, d);
                }
            ...
            for(var e : damages){
                ...
                if(build != null && !((build instanceof TantalumWall.TantalumWallBuild))){
                    if(source != null){
                        if(source.type.status != StatusEffects.corroded) build.damage(source, team, e.value);
                    }else{
    ...
    private static void completeDamage(Team team, float x, float y, float radius, float damage){
        ...
        for(int dx = -trad; dx <= trad; dx++){
            for(int dy = -trad; dy <= trad; dy++){
                ...
                if(tile != null && tile.build != null && !(tile.build instanceof TantalumWall.TantalumWallBuild) && (team == null || team != tile.team()) && dx*dx + dy*dy <= trad*trad){
