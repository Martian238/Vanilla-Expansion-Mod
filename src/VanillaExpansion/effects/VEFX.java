...
import VanillaExpansion.expand.graphics.*;
import mindustry.*;
...
public class VEFX{
    public static Effect shield = new Effect(30f, e -> {
        ...
        color(Tmp.c1.set(VEPal.primary).a(Mathf.absin(e.fin(pow2Out), 1f / 50f, 1f) * 0.5f * e.fout()));
    ...
    aoeExplosion2 = new Effect(80f, 500f, e -> {
        ...
        z(z);
        color(VEPal.primary, Pal.lightOrange, Color.gray, e.fin());
    ...
    apathyCrit = new Effect(80f, e -> {
        ...
        for(int i = 0; i < 45; i++){
            ...
            color(VEPal.primary, VEPal.blood, pow2.apply(time));
    ...
    bigLaserHitSpark = new Effect(15f, e -> {
        color(Color.white, VEPal.primary, e.fin());
        ...
        for(int i = 0; i < 6; i++){
            ...
            if(f > 0.001f){
                color(Color.white, VEPal.primary, f);
    ...
    bigLaserHit = new Effect(30f, e -> {
        color(Color.white, VEPal.primary, Color.gray, pow2Out.apply(e.fin()));
    ...
    destroySparks = new Effect(40f, 1200f, e -> {
        ...
        for(int i = 0; i < isize2; i++){
            ...
            if(f < 1){
                color(VEPal.paleYellow, Pal.lightOrange, Color.gray, f);
        ...
        for(int i = 0; i < isize; i++){
            ...
            if(f < 1){
                color(VEPal.paleYellow, Pal.lightOrange, Color.gray, pow2In.apply(f));
                Lines.lineAngle(v.x + e.x, v.y + e.y, v.angle(), (size / 3f) * rsize * (1 - f));
            }
        }

        color(VEPal.paleYellow);
        for(int i = 0; i < 4; i++){
            float rot = i * 90f;
    ...
    empathyParryExplosion = new Effect(40f, e -> {
        color(VEPal.empathyDark, e.fout());
        blend(Blending.additive);
    ...
    empathyPrimeStrike = new Effect(40f, 300f, e -> {
        ...
        Tmp.c1.set(VEPal.empathyAdd).a(Mathf.clamp((e.lifetime - e.time) / 30f));
        color(Tmp.c1);
        blend(Blending.additive);
    ...
    empathyLightningHit = new Effect(14f, e -> {
        ...
        color(Color.white, VEPal.empathy, e.fin());
    ...
    empathyBlast = new Effect(60f, 900f, e -> {
        Draw.color(Color.white, VEPal.empathyAdd, pow2Out.apply(e.fin()));
    ...
    empathyBigLaserHit = new Effect(30f, e -> {
        color(Color.white, VEPal.empathy, Color.gray, pow2Out.apply(e.fin()));
    ...
    empathyDepowered = new Effect(40f, 1200f, e -> {
        ...
        color(VEPal.red, VEPal.empathyAdd, e.fin());
        blend(Blending.additive);
    ...
    endDeath = new Effect(50f, 1000f, e -> {
        ...
        Draw.color(VEPal.darkRed, VEPal.empathy, Mathf.curve(pow2Out.apply(fin1), 0f, 0.5f));
        ...
        Draw.color(VEPal.darkRed, VEPal.empathy, Mathf.curve(pow2Out.apply(e.fin()), 0f, 0.5f));
    ...
    coloredHit = new Effect(15f, e -> {
        ...
        color(Color.white, VEPal.red, e.fin());
        Lines.stroke(0.5f + e.fout());
    ...
    desGroundHit = new Effect(30f, 250f, e -> {
        ...
        color(VEPal.melt, e.color, pow3Out.apply(e.fin()));
    ...
    desRailHit = new Effect(80f, 900f, e -> {
        ...
        ang = 180f;
        rscl = 0.5f * sizeScl;
        Draw.color(VEPal.red, Color.white, e.fin());
        Lines.stroke(3f);
        for(int i = 0; i < 7; i++){
            int count = 12;
            for(int j = 0; j < count; j++){
    ...
    desNukeShoot = new Effect(35f, e -> {
        ...
        for(int i = 0; i < 5; i++){
            for(int j = 0; j < 7; j++){
                ...
                Draw.color(VEPal.red, Color.white, f);
    ...
    public static Effect nuclearcloud = new Effect(120f, 1500f, e -> {
        ...
        // 辐射粒子效果
        Draw.color(VEPal.melt, Color.gray, e.fout());
        for(int i = 0; i < 80; i++){
